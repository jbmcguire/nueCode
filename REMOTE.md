...
Use this when you want to connect to a nueCode server from another device such as a phone, tablet, or separate desktop app.
...
Run the server with `nuecode serve`.

```bash
npx nuecode serve --host "$(tailscale ip -4)"
```

`nuecode serve` starts the server without opening a browser and prints:
...
Use `nuecode serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `nuecode project ...` on the server machine instead.
...
1. `nuecode serve` issues a one-time owner pairing token.
...
Use `nuecode auth` to manage access after the initial pairing flow.
...
Use `nuecode auth --help` and the nested subcommand help pages for the full reference.
...
- Use `nuecode auth` to revoke credentials or sessions you no longer trust.
