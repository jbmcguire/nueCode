...
const APP_USER_MODEL_ID = isDevelopment ? "com.t3tools.nuecode.dev" : "com.t3tools.nuecode";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "nuecode-dev.desktop" : "nuecode.desktop";
const LINUX_WM_CLASS = isDevelopment ? "nuecode-dev" : "nuecode";
const USER_DATA_DIR_NAME = isDevelopment ? "nuecode-dev" : "nuecode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "nueCode (Dev)" : "nueCode (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.NUECODE_PORT;
  delete env.NUECODE_MODE;
  delete env.NUECODE_NO_BROWSER;
  delete env.NUECODE_HOST;
  delete env.NUECODE_DESKTOP_WS_URL;
  delete env.NUECODE_DESKTOP_LAN_ACCESS;
  delete env.NUECODE_DESKTOP_LAN_HOST;
  return env;
}
...
function resolveAdvertisedHostOverride(): string | undefined {
  const override = process.env.NUECODE_DESKTOP_LAN_HOST?.trim();
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { nuecodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.nuecodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.NUECODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("nueCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.NUECODE_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.NUECODE_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `nueCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/nueCode (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`nuecode`). If the legacy
...
function configureAutoUpdater(): void {
  const githubToken =
    process.env.NUECODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.NUECODE_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.NUECODE_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
async function bootstrap(): Promise<void> {
  ...
  const configuredBackendPort = resolveConfiguredDesktopBackendPort(process.env.NUECODE_PORT);
  if (isDevelopment && configuredBackendPort === undefined) {
    throw new Error("NUECODE_PORT is required in desktop development.");
  }
