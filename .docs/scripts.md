...
- Dev commands default `NUECODE_STATE_DIR` to `~/.nuecode/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.nuecode-2`
...
- Desktop production windows load the bundled UI from `nuecode://app/index.html` (not a `127.0.0.1` document URL).
...
Set `NUECODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `NUECODE_DEV_INSTANCE`)
- Example: `NUECODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `NUECODE_PORT_OFFSET` to a numeric offset.
