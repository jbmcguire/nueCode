...
export const DEFAULT_NUECODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".nuecode"),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "dev", "--filter=nuecode"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("NUECODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("NUECODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid NUECODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `NUECODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric NUECODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed NUECODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_NUECODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly nuecodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(nuecodeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      NUECODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.NUECODE_PORT = String(serverPort);
      ...
      delete output.NUECODE_MODE;
      delete output.NUECODE_NO_BROWSER;
      delete output.NUECODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.NUECODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.NUECODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.NUECODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.NUECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.NUECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.NUECODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.NUECODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.NUECODE_MODE = "web";
      delete output.NUECODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read NUECODE_PORT_OFFSET/NUECODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.NUECODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.NUECODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  nuecodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all nueCode data (equivalent to NUECODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("NUECODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to NUECODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("NUECODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to NUECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("NUECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to NUECODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("NUECODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to NUECODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("NUECODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to NUECODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("NUECODE_PORT")),
  ),
