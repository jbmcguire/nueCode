...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "nueCode (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "nueCode (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                NUECODE_DESKTOP_SKIP_BUILD: "true",
                NUECODE_DESKTOP_KEEP_STAGE: "true",
                NUECODE_DESKTOP_SIGNED: "true",
                NUECODE_DESKTOP_VERBOSE: "true",
                NUECODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
