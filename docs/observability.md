...
nueCode has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.nuecode/userdata/logs/server.trace.ndjson`).
...
```bash
export NUECODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export NUECODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export NUECODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export NUECODE_TRACE_MIN_LEVEL=Info
export NUECODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `NUECODE_OTLP_*`.

macOS app bundle example:

```bash
NUECODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
NUECODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
NUECODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/nueCode.app/Contents/MacOS/nueCode"
```
...
```bash
tail -f "$NUECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$NUECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$NUECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `NUECODE_OTLP_TRACES_URL` was not set
...
- `NUECODE_TRACE_FILE`: override trace file path
- `NUECODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `NUECODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `NUECODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `NUECODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `NUECODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `NUECODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `NUECODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `NUECODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `NUECODE_OTLP_SERVICE_NAME`: service name, default `nuecode-server`
